...
# Module:       ocean_360_border.py
# Description:  Tracking using Ocean tracker with left/right border improvement
#################################################################################################
# ------------------------------------------------------------------------------
...
class Ocean360Border:
    """Tracking using Ocean tracker with left/right border improvement"""
    def __init__(self, resume: str, video_path: str, groundtruth_path: str = None, save_result_path: str = None):
        ...
        self.WINDOW_NAME_BORDER = "Tracker-Ocean-frame_shifted"
    ...
    def run_ocean_border(self):
        """Method for start Ocean tracking with improvement of object crossing left/right border in equirectangular projection"""
    ...
    def _track_video(self, siam_tracker, online_tracker, siam_net):
        ...
        print("--------------------------------------------------------------------")
        print("Ocean tracking process with border improvement has started...")
        print("Tracker  : Ocean")
        ...
        while True:
            ...
            # update tracker
            if self.online:
                rgb_im = cv2.cvtColor(frameShifted, cv2.COLOR_BGR2RGB)
                state = online_tracker.track(frameShifted, rgb_im, siam_tracker, state)
            else:
